...
def master():
    ...
    kickstartScripts = ["main", "network", "storage", "packages", "post-nochroot", "post-chroot"]
    templateSubfoler = ""
    outDir = ""
...
def kickstartGenerator(nodeType, menuData):
    ...
    for index in range(len(kickstartScripts)):
        config_template = env.get_template(kickstartScripts[index] + '.ks')
        config_rendered = config_template.render(data=menuData)
        fileName = KICKSTART_OUTPUT_DIRECTORY + outDir + kickstartScripts[index] + ".ks"
